...
import { ApiError, type Anchor, type ChunkOverrideResponse, type ChunkStudioProjection, type CurrentSession, type RetrievalExperiment, type RetrievalSide, type Space, apiFetch, getCurrentSession } from "./api";

type View = "studio" | "playground";
...
async function createOverride(): Promise<void> {
  ...
  studioLoading.value = true;
  try {
    const response = await apiFetch<ChunkOverrideResponse>(`/api/v1/spaces/${escapePathSegment(selectedSpaceId.value)}/chunk-studio/children/${escapePathSegment(projection.value.childChunkId)}/overrides`, { method: "POST", body: { documentRevisionId: projection.value.documentRevisionId, contentRef: projection.value.contentRef, textHash: projection.value.textHash, reason: overrideReason.value.trim() } });
...
async function transitionOverride(targetState: "ACTIVE" | "NEEDS_REVIEW" | "DISCARDED"): Promise<void> {
  studioNotice.value = ""; studioError.value = "";
  ...
  studioLoading.value = true;
  try {
    const response = await apiFetch<ChunkOverrideResponse>(`/api/v1/spaces/${escapePathSegment(selectedSpaceId.value)}/chunk-studio/children/${escapePathSegment(projection.value.childChunkId)}/overrides/${escapePathSegment(current.overrideId)}/transitions`, { method: "POST", body: { targetState, expectedVersion: current.version, reason: transitionReason.value.trim() } });
  ...
  <main class="shell">
    ...
    <section v-if="view === 'studio'" role="tabpanel" aria-labelledby="chunk-studio-tab" class="view-section">
      ...
      <template v-if="projection">
        ...
        <div class="two-column action-grid"><form class="card action-card" @submit.prevent="createOverride"><div class="card-title"><h3>创建 manual override</h3><span class="tag">服务端生成审计字段</span></div><p class="muted">客户端只提交当前 revision 的 contentRef、textHash 和 reason；不提交 state、createdBy、时间戳或正文。</p><div class="field"><label for="override-reason">创建 reason</label><textarea id="override-reason" v-model="overrideReason" rows="3" maxlength="2000" placeholder="说明为何需要人工 override"></textarea></div><button type="submit" :disabled="studioLoading || !canEdit">{{ studioLoading ? "提交中…" : "创建 override" }}</button><p v-if="isViewer" class="permission-hint">VIEWER 无写权限，按钮已禁用；API 仍是最终权限边界。</p></form><form class="card action-card" @submit.prevent="transitionOverride('ACTIVE')"><div class="card-title"><h3>冲突与状态流转</h3><span v-if="projection.override.state === 'NEEDS_REVIEW'" class="conflict-badge">需要复核</span></div><p class="muted">允许 ACTIVE → NEEDS_REVIEW → ACTIVE / DISCARDED；DISCARDED 为终态。</p><div class="field"><label for="transition-reason">流转 reason</label><textarea id="transition-reason" v-model="transitionReason" rows="3" maxlength="2000" placeholder="记录复核结论"></textarea></div><div class="button-row"><button type="submit" :disabled="studioLoading || !canEdit || projection.override.state !== 'NEEDS_REVIEW'">恢复 ACTIVE</button><button type="button" class="danger-button" :disabled="studioLoading || !canEdit || projection.override.state !== 'NEEDS_REVIEW'" @click="transitionOverride('DISCARDED')">DISCARDED</button><button type="button" class="secondary-button" :disabled="studioLoading || !canEdit || projection.override.state !== 'ACTIVE'" @click="transitionOverride('NEEDS_REVIEW')">标记复核</button></div><p v-if="projection.override.state === 'DISCARDED'" class="permission-hint">DISCARDED 是终态，契约不允许恢复。</p></form></div>
    ...
    <section v-else role="tabpanel" aria-labelledby="retrieval-playground-tab" class="view-section">
...
@media (max-width: 850px) { .shell { width: min(100% - 28px, 1220px); padding-top: 32px; } .app-header, .section-heading, .workspace-bar, .lookup-form { flex-direction: column; align-items: stretch; } .status-chip { align-self: flex-start; } .identity-grid, .two-column, .side-grid, .form-grid { grid-template-columns: 1fr; } .nested-fields { max-width: none; } .result-summary { flex-direction: column; } .safe-result { align-items: flex-start; } } @media (max-width: 520px) { .shell { width: min(100% - 20px, 1220px); } .main-nav { overflow-x: auto; } .main-nav button { white-space: nowrap; } .form-actions { flex-direction: column; align-items: flex-start; } .details { grid-template-columns: 1fr; gap: 4px; } .details dd { margin-bottom: 5px; } }
</style>
