...
<template>
  <main class="shell">
    ...
    <BusinessFlowView v-if="session && spaces.length > 0 && view === 'flow'" :selected-space-id="selectedSpaceId" :current-role="currentRole" :current-user-id="session.user.userId" @start-answer="(config) => { answerDefaults = config; view = 'answer'; }" @open-control="openControl" />
    ...
    <ControlCenterView v-else-if="session && spaces.length > 0 && view === 'control'" role="tabpanel" aria-labelledby="control-center-tab" :selected-space-id="selectedSpaceId" :current-role="currentRole" :current-user-id="session.user.userId" :initial-section="controlSection" :initial-run-id="lastRunId" />
    <AnswerView v-else-if="session && spaces.length > 0 && view === 'answer'" role="tabpanel" aria-labelledby="answer-tab" :selected-space-id="selectedSpaceId" :defaults="answerDefaults" @run-created="lastRunId = $event" />
  </main>
</template>
