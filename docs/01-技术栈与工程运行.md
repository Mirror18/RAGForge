...
本文件是 RAGForge 的技术栈、工程结构和本地启动主文档。它先回答“用什么技术、代码在哪里、如何启动”，再说明共享目录为何保留以及当前下一步如何定位。
...
| 层次 | 技术/版本基线 | 实际职责 |
|---|---|---|
| 后端业务 | Java 21、Spring Boot 3.5.x、Maven 多模块 | `backend/server` 承担同步 API、认证授权、空间边界、问答编排和管理查询 |
| 异步摄取 | Java 21、Spring Boot Worker、RabbitMQ | `backend/ingestion-worker` 承担来源同步、解析、分块、embedding、索引构建和失败重投 |
| AI Runtime | Python、FastAPI/uv 运行面 | `backend/ai-runtime` 仅承担 OCR 与 rerank，不承担第二套业务后端职责 |
| Web | Vue、TypeScript、Vite、Vitest、Playwright | `frontend/ragforge-web` 承担用户登录、空间、来源、索引、问答和管理界面 |
| 持久化 | PostgreSQL、Flyway、Valkey | PostgreSQL 保存业务状态和审计；Flyway 维护追加式迁移；Valkey 支持 Session、缓存和事件协作 |
| 检索/对象 | Qdrant、MinIO/S3-compatible object store | Qdrant 保存 dense/lexical 检索事实；MinIO 保存不可变 revision/artifact material |
| 质量与交付 | Python CI scripts、Docker Compose、GitHub Actions、Syft/Grype | 契约、架构、格式、秘密、评估、镜像、SBOM 和部署门禁 |
| 外部模型 | 本地 Ollama；经空间授权的 OpenAI-compatible Provider | 默认 LOCAL_ONLY；云端调用必须显式 opt-in，禁止本地失败静默切云 |

依赖版本和许可证以根 `pom.xml`、前端 lockfile 及 [`06-安全合规与研究.md`](./06-安全合规与研究.md) 为准；历史版本和执行过程不作为当前运行入口。
...
| 问题 | 入口 |
|---|---|
| 我们做什么 | [项目章程](./03-功能与用户结果.md) → [PRD](./03-功能与用户结果.md) |
| 代码为什么这样分 | [总体架构](./02-项目架构与领域模型.md) → [ADR](./07-架构决策记录.md) |
...
更完整的目录职责、启动方式和文档顺序见[项目手册](./01-技术栈与工程运行.md)。
...
前置：Docker Desktop、Java 21、Maven、Node.js 和 Python。Ollama 不是项目启动前置条件；只有进行真实本地模型验收时才需要它。

```powershell
.\scripts\dev\start-local.bat
```

脚本会按隔离项目名启动 PostgreSQL、Qdrant、RabbitMQ、Valkey、MinIO，再启动 Server、Worker 和 Web。默认不会检查或连接 Ollama；需要真实本地模型验收时可用 `-CheckOllama` 显式检查所需模型。日志放在被忽略的 `tmp/local-run/`；不需要 Web 时可用 `-SkipWeb`。
...
文档按 `docs/00–03` 主题文档 → `术语与概念字典` → `docs/04–07` 工程治理主文档 → `docs/08–14` Agent/治理文档的顺序组织。`docs/README.md` 是索引，本手册是技术栈与运行总览；每份文档只维护自己的权威事实。

写新内容时遵循三条规则：

1. 产品范围先写[功能与用户结果](./03-功能与用户结果.md)；架构变化先写[架构决策记录](./07-架构决策记录.md)；当前状态、风险和追溯写入 `docs/08–13`，历史执行细节写 Git commit body。
...
如果你不知道下一步做什么，先读 [`docs/00-需求处理与最终效果.md`](./00-需求处理与最终效果.md)，再读本技术栈与运行文档。当前主线在 Phase 7 P2，P0/P1 功能与回归门禁已完成，P7D-03 因缺少独立 Ubuntu 24.04 环境而阻塞；不要把历史记录或页面文案当作部署验收证据。
...
目录取舍、职责矩阵、启动顺序和文档阅读顺序见[项目手册](./01-技术栈与工程运行.md)。
...
- [项目手册](./01-技术栈与工程运行.md)
- [项目章程](./03-功能与用户结果.md)
- [总体架构](./02-项目架构与领域模型.md)
- [架构演进与 ADR-0013](./02-项目架构与领域模型.md)
...
前置条件：Docker Desktop、Java 21、Maven 和 Node.js；Ollama 仅在需要真实本地模型验收时使用。
