...
依赖版本、许可证和运行边界见 [`18-阶段1依赖台账.md`](./18-阶段1依赖台账.md) 与 [`20-阶段3依赖与许可证证据.md`](./20-阶段3依赖与许可证证据.md)；它们是证据记录，不替代本节的技术栈总览。
...
文档按 `docs/00–03` 主题文档 → `术语与概念字典` → `docs/04–07` 工程治理主文档 → `docs/08–33` 项目记录的顺序组织。`docs/README.md` 是索引，本手册是技术栈与运行总览；每份文档只维护自己的权威事实。

写新内容时遵循三条规则：

1. 产品范围先写[项目范围与产品闭环](./03-功能与用户结果.md)；架构变化先写[架构决策记录](./07-架构决策记录.md)；阶段状态、风险、追溯和证据摘要写入 `docs/08–32` 对应编号文档。
