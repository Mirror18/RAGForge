...
Ollama 是可选的宿主机服务，仅在选择本地模型时通过明确地址连接；项目启动不要求 Ollama 已运行，生产可替换为局域网推理服务。Compose 只保存非敏感默认值，Secret 由环境或 secret files 注入。
...
本地启动不依赖 bootstrap；若当前只验证服务可用，可以保持 Token 未配置，状态会是 `required=true`、`available=false`。Windows 启动脚本不会自动生成、注入或保存该 Token，也不会自动创建平台管理员。
