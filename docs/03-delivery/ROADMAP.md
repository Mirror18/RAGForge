...
实施前按看板逐卡补齐 token 预算、精确 ownership/read_only 和实际测试命令；不把整份提案作为一个实现任务。性能预算与发布阈值先按现行质量门禁固定并记录，达标证据产生前不宣称优化收益。迁移与回滚细节见[架构演进](../02-architecture/ARCHITECTURE_EVOLUTION.md)。
