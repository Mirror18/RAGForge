...
这里记录“应该怎样交付”和“达到什么门槛”；某张任务卡实际改了什么、运行了什么命令、结果是什么，仍回到 `docs/09-项目任务总台账.md` 和 Git history 查证。
...
- `RAG_EVALUATION.md`
- `PERFORMANCE_PLAN.md`
- `PHASE_6_CAPACITY_RUNBOOK.md`
- `TEST_DATA_POLICY.md`
- `CONTRIBUTING.md`

---

## 原始文档：`AGENT_LOOP_PROMPT.md`
...
> 状态真源：[`docs/08-项目状态卡.md`](./08-项目状态卡.md)
> 任务预算看板：[`docs/09-项目任务总台账.md`](./09-项目任务总台账.md)
> Worker Ticket 模板：[`TICKET_TEMPLATE.yaml`](../.agents/tickets/TICKET_TEMPLATE.yaml)
...
(1) 读 docs/08-项目状态卡.md（唯一状态入口）
(2) 读 docs/09-项目任务总台账.md（任务与预算看板）
仅当出现「状态卡 vs. 代码事实」矛盾，或你要执行阶段闭环 / 发布验收时，才打开 docs/10-项目状态审计记录.md；日常分派不要读它。
...
二、为每张候选卡片生成 Worker Ticket（YAML，写入 .agents/tickets/<CARD_ID>-<agent>.yaml）
...
- scope.read_only：**精确到具体文件名**，给最少必要的 5–15 个路径。禁止把 "contracts/"、"backend/server/"、"docs/02-项目架构与领域模型.md" 这种整目录放进去。如果 Worker 真的需要更多文件，它会停下来请求扩展，而不是你预先塞满。
- scope.forbidden：显式列出项目状态卡 / 项目状态审计记录 / 风险登记表 / 需求追溯矩阵 / AGENTS.md / Agent 工程记忆 / 项目任务总台账这些 Worker 绝对不许动的文件；以及所有其它 ownership 之外的模块。
...
- 给每个 Worker Agent 的指令只有两件事：(a) 角色用「§2 Worker 专用提示词」；(b) 让它先读 .agents/tickets/<...>.yaml。不要把本 Orchestrator 提示词也塞给 Worker。
...
- 合并完一批（1–3 张）后：更新 08-项目状态卡.md §1 最新 main SHA；更新 §6 分派表的 状态/实际消耗 / 完成SHA / 备注。其它章节不要改。
...
> 适用场景：Orchestrator 分派了一张具体卡片（例如 P7C-04），并在 `.agents/tickets/` 下生成了 YAML Ticket。
>
...
启动前只做 3 件事：
(1) 读 .agents/tickets/<你的 Ticket 文件路径> —— 这是你唯一的任务书 + 验收真源。
...
你本次的唯一目标：<由调用方填写，例如："对 Phase 7 做代码 vs 04-交付路线与质量门禁.md 对应章节的审计，并输出 09-项目任务总台账新卡片列表" 或 "校正 08-项目状态卡与代码事实的漂移">
...
2. 精确到卡片级的任务清单：每张卡片含 ID、标题、依赖、Ownership、Token 预算、验收输出、必跑测试（格式对齐 09-项目任务总台账.md）。
3. 若本轮任务是「校正状态卡漂移」：直接输出 08-项目状态卡.md 的更新 diff 建议与 09-项目任务总台账.md 的更新 diff 建议。
4. 风险：识别出新的高 / 中 / 低风险条目或确认 11-风险登记表.md 中状态变化。
...
- 所有质量和隔离门槛达到 [项目章程](./03-功能与用户结果.md)。
...
状态（2026-08-29）：`implementation-reconciliation`。本阶段先依据代码与可执行门禁补齐 MVP 断点，再进入 Linux 发布：平台初始化、Provider 能力实测、生成 streaming/cancel、Git 来源、反馈/审计、durable retrieval/rerank 和自动化测试均不能因已有契约或页面文字而视为完成。随后才执行容器加固、干净 Ubuntu、升级/回滚、SBOM/扫描和公共化检查。当前任务编号和依赖以 [`09-项目任务总台账.md`](./09-项目任务总台账.md) 为准，具体执行细节写入对应 Git commit。
...
版本：`knowledge-evolution-roadmap.v1`。本轮只完成 [ARCH-DOC-01](./09-项目任务总台账.md) 文档卡；[ADR-0013](./07-架构决策记录.md)已于 2026-09-05 接受，Phase 7 已有退出条件和 Ubuntu 外部验收依赖保持原约束。以下为待拆分工作包，不是已满足依赖的 Worker Ticket。
...
实施前按看板逐卡补齐 token 预算、精确 ownership/read_only 和实际测试命令；不把整份提案作为一个实现任务。性能预算与发布阈值先按现行质量门禁固定并记录，达标证据产生前不宣称优化收益。迁移与回滚细节见[架构演进](./02-项目架构与领域模型.md)。

---

## 原始文档：`DEFINITION_OF_DONE.md`
...
## 原始文档：`PHASE_0_CHECKLIST.md`

### Phase 0 执行清单：竞品基准与许可证闸门

#### 1. 实验资产

- [x] 建立 30–50 份公开/合成 benchmark corpus；实际 36 份，详细结果保留在 Git history 与 `tests/evidence/`。
...
- [x] 填写 [基准结果表](./06-安全合规与研究.md)；逐案例证据保留在 Git history 与 `tests/evidence/`。
...
- [x] 形成阶段退出结论；执行复盘保留在对应 Git commit。
- [x] 项目负责人确认进入 Phase 1；状态记录已切换为 Phase 1 Ready。

---

## 原始文档：`PHASE_1_CHECKLIST.md`

### Phase 1 执行清单：工程与领域骨架

本清单把 [`docs/04-交付路线与质量门禁.md`](./04-交付路线与质量门禁.md) 的工程退出条件拆成可执行证据。`[x]` 只表示已有可复核证据；“待外部 CI”不以文档声明替代运行结果。具体实现和验证过程见对应 Git commits。
...
- [x] `10-项目状态审计记录.md`、`11-风险登记表.md`、`12-需求追溯矩阵.md` 已更新；证据：本次阶段记录提交。
- [x] Phase 1 退出结论已记录；事实、质量/安全数据、问题和后续入口见对应 Git commits 与当前治理文档。
- [x] 阶段正式关闭：Run 31616214088 已成功，未发现新的 P0/P1 代码缺陷；本清单与阶段证据已由中文 closure commit 固化。

---

## 原始文档：`PHASE_2_CHECKLIST.md`
...
- 本清单与 `contracts/openapi/`、`contracts/events/`、`docs/02-项目架构与领域模型.md` 共同定义 Phase 3 合同；未勾选项不得被解释为运行时能力已上线。
...
## 原始文档：`PHASE_4_CHECKLIST.md`

### Phase 4 Chunk Studio、索引与检索 Checklist

状态：`accepted`（2026-08-21）。任务目标与依赖见 [`09-项目任务总台账.md`](./09-项目任务总台账.md)。技术基线维持 Java 21 + Spring Boot 3.5.x（[ADR-0002](./07-架构决策记录.md)）。
...
- Retrospective：任务复盘保留在对应 Git commits。
...
- 任务定义：[`09-项目任务总台账.md`](./09-项目任务总台账.md)
...
## 原始文档：`PHASE_6_CAPACITY_RUNBOOK.md`
