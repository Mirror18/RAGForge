...
- [x] 建立 30–50 份公开/合成 benchmark corpus；实际 36 份，见 [`17-阶段0基准结果.md`](./17-阶段0基准结果.md)。
...
- [x] 填写 [基准结果表](./06-安全合规与研究.md) 并保留 [`逐案例证据`](./17-阶段0基准结果.md)。
...
- [x] 形成 [Phase 0 retrospective](./26-阶段0复盘.md)。
...
本清单把 [`docs/04-交付路线与质量门禁.md`](./04-交付路线与质量门禁.md) 的 Phase 1 退出条件拆成可执行证据。`[x]` 只表示已有可复核证据；“待外部 CI”不以文档声明替代运行结果。完整结果见 [`19-阶段1实现结果.md`](./19-阶段1实现结果.md)。
...
- [x] Phase 1 retrospective 记录事实、质量/安全数据、问题和 Phase 2 入口；证据：[`27-阶段1复盘.md`](./27-阶段1复盘.md)。
...
状态：`accepted`（2026-08-21）。执行计划与所有权见 [22-阶段4执行计划.md](./22-阶段4执行计划.md)。技术基线维持 Java 21 + Spring Boot 3.5.x（[ADR-0002](./07-架构决策记录.md)）。
...
- Retrospective：[`31-阶段5复盘.md`](./31-阶段5复盘.md)。
...
- 执行计划：[`25-阶段7执行计划.md`](./25-阶段7执行计划.md)
