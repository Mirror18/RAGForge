...
脚本使用 Compose project `ragforge-p6-capacity-a1`、Qdrant `26337/26338`、独立 volume/network，并在退出时执行 `docker compose ... down -v`。如中断，先确认仅删除该 project 的资源，再执行同一命令重试。默认 1,000,000 points、20 并发、400 次混合过滤查询；结果包含 p50/p95/p99、吞吐、错误率、Recall@10、Ollama 响应 hash、配置 hash、脚本 hash、代码 commit 和机器信息。
