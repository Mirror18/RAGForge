...
研究结论是设计输入，不等于已引入依赖或已获得许可证；安全文档中的“计划”“建议”“待复核”也不等于已完成控制。当前状态以 `docs/08–13` 的治理记录和 Git history 为准。

## 收录源文件

- `REPOSITORY_LICENSING.md`
- `SECURITY_BASELINE.md`
- `THREAT_MODEL.md`
- `DATA_EGRESS_AND_RETENTION.md`
- `OSS_COMPLIANCE.md`
- `GITHUB_BENCHMARK.md`
- `REFERENCES.md`
- `UPSTREAM_REUSE_REGISTER.md`
- `2026-09-05-knowledge-architecture-benchmark.md`
- `SECURITY.md`
- `THIRD_PARTY_NOTICES.md`

---

## 原始文档：`REPOSITORY_LICENSING.md`
...
3. 本文件中的第三方说明、`licenses/`、SBOM 和复用登记一致。
...
## 原始文档：`DATA_EGRESS_AND_RETENTION.md`
...
- 复制源码：`third_party/`、`licenses/`、本文件的第三方说明章节、复用登记表四处一致。
...
完整逐案例结果、运行日志摘要、生命周期演练和失败证据保留在 `tests/evidence/` 与对应 Git commits。本节只保留可用于当前设计判断的汇总，避免把不同产品的不可比指标合并成一个总分。
...
4. 竞品实验没有改变 RAGForge 的模块化单体 + 独立 ingestion worker 技术基线；后续任务入口只看 [`09-项目任务总台账.md`](./09-项目任务总台账.md)，未决风险看 [`11-风险登记表.md`](./11-风险登记表.md)。

#### 2026-09-05 增量架构调研

新增 [知识库架构对标与取舍](./06-安全合规与研究.md)，研究 RAGFlow、Dify、Onyx，并以 Haystack 补充组件化流水线参考。历史 Phase 0 的版本、许可证与复用决定仍属于当时记录；本次 HEAD 观察不更新旧依赖版本或批准范围。设计结果见[架构演进提案](./02-项目架构与领域模型.md)。
...
## 原始文档：`UPSTREAM_REUSE_REGISTER.md`
...
截至 `2026-08-12`，RAGForge 根仓库没有引入上述依赖、没有 vendored third-party source，也没有复制第三方许可证全文。正式引入前必须重新核对上游精确版本、文件级许可证、传递依赖/SBOM、Notice/copyright 和适用的商业/分发义务，并同步更新本文件的第三方说明章节。
...
## 原始文档：`2026-09-05-knowledge-architecture-benchmark.md`
...
- 本记录没有引入依赖、复制源码、接受许可证或证明性能收益。设计入口为[演进提案](./02-项目架构与领域模型.md)。
