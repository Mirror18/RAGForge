...
| GOV-03 | 文档实际内容合并与记录归档 | 项目说明按 01–07 顺序阅读；状态、任务、风险、证据和复盘统一进入 08–32 号项目记录；原文与执行细节可由 Git history 追溯 | ✅ 完成 |
| GOV-04 | docs 与根目录最终扁平化 | `docs/` 只保留编号文档和 README；Worker Ticket 与机器证据分别归入 `.agents/tickets/`、`tests/evidence/`；根目录只保留硬规则、项目入口和变更日志 | ✅ 完成 |
...
| 卡片 ID | 标题 | 前置 | Ownership | Token 预算 | 验收输出 | 必跑测试/门禁 |
|---|---|---|---|---:|---|---|
| GOV-04 | 按 dataH 风格完成最终归拢：删除 `docs/` 下残留目录；把状态、任务、审计、风险、追溯、记忆、阶段计划、证据摘要和复盘改为 `docs/08–32` 平铺文件；Worker Ticket 放入 `.agents/tickets/`；根目录仅保留 `AGENTS.md`、`README.md`、`CHANGELOG.md` 三类项目文档入口，GitHub 标准入口放入 `.github/` | GOV-03 | `docs/`、`.agents/tickets/`、`.github/`、`AGENTS.md`、`README.md`、`.agents/skills/`、`scripts/`、`licenses/` | **8,000** | `docs/` 无子目录；根目录无散落项目说明文档；所有迁移文件保留实际正文；相对链接、路径索引、格式、架构、秘密、观测门禁通过；下一步仍能从状态卡→任务台账→Ticket 定位；执行细节仍由 Git history 保存 | `check_markdown_links.py`、`format_check.py`、`architecture_check.py`、`path_index_check.py`、`secret_scan.py`、`observability_check.py`、`git diff --check` |
