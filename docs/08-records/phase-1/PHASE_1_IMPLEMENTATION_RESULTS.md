...
| 退出条件 | 状态 | 可复核证据 |
|---|---|---|
| 新环境按文档可重复启动 | 已满足（本地） | [`deploy/compose/README.md`](../../05-部署运维与恢复.md)；`python scripts/ci/validate_compose.py --project-name ragforge-p1-orch-check --env-file deploy/compose/env.example`；隔离项目 `ragforge-p1-api-check` 实际启动 PostgreSQL、Qdrant、RabbitMQ、Valkey、MinIO，并使用独立 network、volume 和端口 block |
