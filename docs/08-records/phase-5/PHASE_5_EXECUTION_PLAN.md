...
- 记录日期：2026-08-21
- 统一 base SHA：阶段实现验收基线 `600960f`；ADR 接受基线 `246f993`
- 主分支：`main`
...
|---|---|---|---|---|---|---|
| P5-A | 冻结 checklist、执行计划、Answer/Citation/Tool/SSE schema、评估 fixture 口径 | `docs/04-交付路线与质量门禁.md` 的 Phase 5 章节、`docs/08-records/phase-5/`、`contracts/answer/`、`contracts/agent/`、`contracts/events/answer*.json`、contract tests | Phase 4 retrieval contracts、PRD、security/evaluation docs | 主 Agent | JSON/schema + contract tests | 运行时实现、migration |
| P5-B | Prompt immutable version、RAG run/step/citation/tool provenance persistence | `backend/server/src/main/resources/db/migration/V11__phase5_*.sql`、`prompt/`、`run/` persistence additions | P5-A contracts、V1–V10、ADR-0005/0008 | Persistence Agent | targeted PostgreSQL integration + unit | Web、SSRF client、评估记录 |
| P5-C/D | RAG generation、context budget、structured refusal、citation token parser/validator | `backend/server/src/main/java/com/ragforge/server/answer/`、`retrieval/` additions、server tests | P5-A/B、RetrievalService、EvidenceBundle、provider egress | Answer Agent | unit/integration/security/evaluation slice | Tool HTTP client、Web |
| P5-E | Answer SSE/replay/cancel and citation/revision API + UI | server answer controller/event projection; `frontend/ragforge-web/src/` answer views/API; owned contract additions only through main | P5-C/D, existing RunEventStore/SSE | Interaction Agent | MockMvc, replay/cancel, web format/build | Migration, model/provider policy |
...
本阶段退出条件已满足：ADR-0010 已由用户接受，方案 A typed authorization context、既有 provider connection、revision/artifact material service 和显式 `LOCAL_ONLY` Ollama route 的真实空间级 RAG E2E 均已验证。默认路径仍 fail-closed；真实云出境、生产容量和大规模质量评估不在本阶段授权范围内，转入 Phase 6。
