...
| P7-WEB-001 | 首次设置与成员协作 | `POST /spaces/{spaceId}/members`、`SpaceService.addMember`、`PersonalSpaceView.vue` 邮箱/角色表单、`SpaceServiceTest`、`ServerIntegrationTest.spaceAdminCanAddActiveMemberByExactEmailWithoutExposingUserDirectory` | 完成：精确邮箱匹配 ACTIVE 用户；Editor/Viewer 首次进入、重复/停用/越权拒绝；单元 3/3、Server 集成 8/8、定向 1/1、Web build 通过 | 不开放平台用户目录；平台 bootstrap 仍见 P7-AUDIT-001 |
| P7-WEB-002 | 来源、任务和索引持续维护 | `BusinessFlowView.vue` 多文件仅逐个提交，只有单文件等待摄取；job/index 使用 `slice(0, 5)`；无重试/同步/归档/回滚入口 | 一次性 happy path，不能维护增长中的知识库 | 来源库、逐项终态、分页/筛选、失败恢复、Git、索引发布与回滚 E2E |
| P7-WEB-003 | 可核验问答与历史 | Citation preview API 返回结构化 provenance；`answer.ts` 丢弃 body，页面明确不渲染正文；无显式新会话和 feedback | 回答可生成，但核验、会话控制和反馈未闭环 | 受权正文/原文跳转、历史 citation、新会话、反馈、streaming/cancel E2E |
| P7-WEB-004 | Chunk/检索/运行上下文导航 | `App.vue` 要求手填 chunk/contentRef/hash/index/profile UUID/version；Run 页面要求手填 Run ID；生产界面暴露 synthetic queryVector | 工程调试入口，不是普通用户工具 | 来源/命中/Citation/Run 间深链，资源选择器，高级诊断与普通路径分离 |
| P7-WEB-005 | 导航、分页与自动化 | `view` 是本地 ref，无 Router；多处固定 limit 100 或前 5 条；`package.json` 无 test script | 刷新丢上下文，大数据量静默遗漏，无回归保障 | URL Router、cursor pagination、规模 fixture、unit/component/E2E 门禁 |
