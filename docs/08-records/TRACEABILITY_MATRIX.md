...
| RF-ANS-001 | context budget、结构化状态、幂等、失败安全 | `RAGAnswerServiceTest`、`AnswerApiControllerTest`、`RevisionArtifactMaterialResolverTest`、V12 answer history | targeted answer tests 15/15；材料 space/revision/ref/hash 不一致拒答；未配置 provider 明确拒答并持久化 | 真实 material service/完整 graph 仍为 R-025 |
| RF-EGR-001 | route/egress 显式授权、无静默 cloud fallback | `Phase5ProviderIntegrationTest`、`ProviderAdapterHttpTest`、Phase 2 egress tests、[`phase5-security.json`](../../tests/evidence/phase5-security.json) | answer/security 19/19；provider HTTP 29/29；未授权云调用 `0` | 真实 provider route/credential 数据端到端验收 |
...
| RF-OBS-001 | run/step/model/tool provenance、历史引用 preview、严格 SSE | V11/V12/V13 migrations、`JdbcAnswerPersistence`、`JdbcRunEventStore`、`AnswerApiController`、`AnswerEventPublisher` | PostgreSQL V1–V13 migration；durable replay/cancel/space isolation 通过；SSE/API targeted tests 7/7 | 真实进程重启演练和过期事件清理仍留后续 |
...
| RF-MAT-001 | revision/artifact service 按 space + revision + immutable parsed artifact 提供证据材料 | [`JdbcRevisionArtifactMaterialService.java`](../../apps/server/src/main/java/com/ragforge/server/ingestion/JdbcRevisionArtifactMaterialService.java)、[`S3ArtifactContentReader.java`](../../apps/server/src/main/java/com/ragforge/server/ingestion/S3ArtifactContentReader.java)、material unit/MinIO integration tests | storage URI 空间前缀、UTF-8 slice、SHA-256 校验和 cancellation/size boundary 通过；显式 object-storage gate | 真实空间级 RAG E2E 与 provider route 配置 |
| RF-GRAPH-001 | 版本化 prompt、active index/profile identity 与真实 answer port graph 的显式组装 | [`Phase5ProductionAnswerConfiguration.java`](../../apps/server/src/main/java/com/ragforge/server/answer/integration/Phase5ProductionAnswerConfiguration.java)、[`JdbcRagPromptTemplateResolver.java`](../../apps/server/src/main/java/com/ragforge/server/prompt/JdbcRagPromptTemplateResolver.java)、[`DeterministicRetrievalVersionIdentityProvider.java`](../../apps/server/src/main/java/com/ragforge/server/answer/integration/DeterministicRetrievalVersionIdentityProvider.java)；server `198/198` | `space_id`/hash 绑定、稳定 evidence identity、默认 fail-closed 与显式 object-storage opt-in 编译/回归通过 | 受控 provider route/credential、真实 embedding/retrieval/generation E2E |
