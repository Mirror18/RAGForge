...
- `R-040` 远程复验增量：Run 33306553953 暴露 checkout 浅克隆无法解析 push base，并非质量指标回归；P7Q-05R 已以完整历史 checkout 修复且保持 gate fail-closed。在重推完成前风险当时保持 `MITIGATING`；最终关闭记录见下一条。
- `R-040` 已关闭：主线 `f0ce7d2` 的 quality Run 33307092918 在统一 preflight 后完成 RAG、完整 Maven、Phase 2–5、安全、SBOM/Grype 与 Web 全门禁，5m48s 全绿；P7D-01 入口前置满足。
...
|---|---|---:|---:|---:|---|---|---|
| R-055 | 阶段恢复复用错误版本的解析产物，导致引用与索引不一致 | 3 | 5 | 15 | 固定输入/配置/产物身份，阶段重投递与并发发布测试；不能只按文档名命中缓存 | Ingestion | OPEN |
| R-056 | 内容删除或成员撤权后，历史证据、预览、缓存或重放绕过当前权限 | 3 | 5 | 15 | 全入口强制 space_id 与当前授权；墓碑优先于索引清理；保留审计身份不重新暴露正文；安全回归 | Security / Retrieval | OPEN |
| R-057 | 新检索计划与在线、调试、评估三条执行路径分叉，产生虚假优化结论 | 3 | 4 | 12 | 同一执行入口和不可变快照；固定数据/索引/模型版本对比；安全零违规、既有质量门禁通过 | Retrieval / Quality | OPEN |
| R-058 | 引入通用 DAG/插件引擎和额外基础设施，超出当前维护与交付能力 | 3 | 4 | 12 | 先固定受控阶段和内部接口，沿用现有部署；新增运行时或依赖另行 ADR/许可证评审 | Architecture | OPEN |
| R-059 | 将上游动态文档或企业版能力误当作当前可复用的开源实现 | 3 | 4 | 12 | 调研记录日期/HEAD观察范围；Onyx EE 与 Dify 附条件明确分开；reference-only，无源码复制或许可证接受 | Compliance | OPEN |
