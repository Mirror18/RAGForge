...
| 审计点 | 基线证据 | 结论与设计影响 |
|---|---|---|
| 分块 | [BusinessIngestionSideEffectHandler](../../backend/ingestion-worker/src/main/java/com/ragforge/ingestion/pipeline/BusinessIngestionSideEffectHandler.java) 第 179–228 行；[摄取文档](../02-architecture/INGESTION_PIPELINE.md) | 该 handler 每文档一个 parent，child 按字符上限和换行切分，无 overlap；语义父块与 token overlap 属于文档目标，不能视为生产 handler 已实现 |
