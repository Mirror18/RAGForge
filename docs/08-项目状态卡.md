...
| GOV-03 | 按实际正文合并 docs 并建立记录归档 | P0 / 用户追加 | ✅ integrated | Orchestrator | main | RAGForge | 10,000 | 未记录 | 见本提交 | 01–07 主文档保留源正文；08–32 号记录保留状态/任务/风险/追溯/证据/复盘；执行细节不复制，回到 Git history |
| GOV-04 | docs 与根目录最终扁平化、分离 Agent Ticket 与机器证据 | P0 / 用户追加 | ✅ integrated | Orchestrator | main | RAGForge | 8,000 | 未记录 | 见本提交 | docs 无子目录；08–32 号记录平铺；Ticket 在 `.agents/tickets/`；根目录仅保留 AGENTS/README/CHANGELOG；执行细节回到 Git history |
