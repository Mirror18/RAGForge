...
|---|---|---|---|---|---|---|
| P4-A | checklist、验收口径、执行记录 | `docs/04-交付路线与质量门禁.md` 的 Phase 4 章节、`docs/22-阶段4执行计划.md` | ROADMAP Phase 4、ADR-0003/0006、[INGESTION_PIPELINE](./02-项目架构与领域模型.md)、[RETRIEVAL_AND_CHAT](./02-项目架构与领域模型.md)、[RAG_EVALUATION](./04-交付路线与质量门禁.md) | 主 Agent | checklist 未勾选、量化门槛明确（Recall@10>=0.90、MRR@10>=0.75）、所有权表提交 | 任何运行时代码 |
