...
- 根目录全量 Maven reactor 的 compile/test 回归（覆盖 `backend/server` 与 `backend/ingestion-worker`），以及 `frontend/ragforge-web` 基于 `package-lock.json` 的 `npm ci`、format/build/test；
...
当前没有 contract test module 时，discover 会报告空骨架并按 workflow 的显式 placeholder 规则处理；引入业务代码后，`frontend/ragforge-web/package-lock.json` 和具体 contract tests 必须一并提交。
